...
int ioption(char inputstring[], char outputstring[]);
int ooption(char inputstring[], char outputstring[]);


HANDLE * createhandle(char voltoopen[]);
...
int main (int argc, char *argv[])
{
    ...
    if(argv[2] == "-i") {
        success = ioption(voltoopen, argv[3]);
    } else if (argv[2] == "-o") {
        success = ooption(voltoopen, argv[3]);
    } else {
...
int ioption(char voltoopen[], char outputstring[])
{
    ...
    HANDLE hvol = createhandle(voltoopen);

    pfile = fopen(outputstring, "rb");
    ...
    case 1: //$Boot
        ...
        if (yorn == 1) {
            altboot(hvol, bpc, pfile, MFTstart, filesize, bps, 1);
        }
    ...
    case 3: //$Upcase
        ...
        if (yorn == 1) {
            altboot(hvol, bpc, pfile, MFTstart, filesize, bps, 2);
        }
    ...
    case 4: //File-record
        ...
        if (yorn == 1) {
            altendofrec(hvol, pfile, MFTstart, filesize);
        }
...
HANDLE * createhandle(char voltoopen[])
{
    ...
    if(hvol == INVALID_HANDLE_VALUE) {
        printf("Couldn't open volume\nAssure ownership of volume and run as administrator. Error code: %d \n", GetLastError());
        return NULL;
    }
    ...
    if(vollocked == 0) {
        printf("Volume could not be accessedfor locking\nError code: %d\n",  GetLastError());
        return NULL;
    }
...
int ooption(char voltoopen[], char outputstring[])
{
    DWORD bytesread, bytesreturned;
    static bool yorn;
    unsigned char sector[512];
    ...
    if(pfile == NULL) {
        printf("Output file failed to open %s\n", outputstring);
        return 1;
    }
...
int retrecord (HANDLE hvol, FILE * pfile, long long int MFTstart)
{
    ...
    if (MFTstart > (pow(2, 32) - 1)) { //More than after 2GB mark
        ...
        lo32 = MFTstart;    //Setup lo32 alone
    }
    ...
    for(a = 0; a < noofrec; a++) {
        ...
        for(b = 0; b < remainingbytes; b++) {
            rectowrite[b] = filerecord[offtoendofrec + b];
        }
...
int retboot (HANDLE hvol, FILE *pfile, int bps, long int bpc, long long int MFTstart, int choice, int drc)
{
    ...
    if (datarunOffset[drc] > (pow(2, 32) - 1)) { //More than after 2GB mark
        ...
        DATAlo32 = datarunOffset[drc];    //Setup lo32 alone
    }

    SetFilePointer(hvol, DATAlo32, &DATAhi32, FILE_BEGIN); //hvol at $DATA

    if (choice != 3) {
        SetFilePointer(hvol, bps, NULL, FILE_CURRENT);    //hvol at $Boot post-VBR
    }
...
void altendofrec (HANDLE hvol, FILE *pfile, long long int MFTstart, long int filesize)
{
    ...
    while (bytesremaining != 0) {
        ...
        if (wsuccess) {
            printf("%d bytes written in filerecord %i\n", byteswritten, frcount);
        }
...
void altbadclus (HANDLE hvol, long int bpc, FILE * pfile, long long int MFTstart, unsigned long clusno, int noofbitstomark, long totclus, int bps, long int filesize)
{
    ...
    if (clusnooffset > (pow(2, 32) - 1)) { //More than after 2GB mark
        ...
        lo32 = clusnooffset;    //Setup lo32 alone
    }
    ...
    if(filerecord[DATAoff + 4] == 0x18 && filerecord[DATAoff + 0x18] == 0x80) {
        DATAoff += 0x18;    //checks first $DATA and moves to second if needed
    }

    if (filerecord[DATAoff + 0x40] == 0x24 && filerecord[DATAoff + 0x42] == 0x42) {
        droffset = DATAoff + 0x48;
    } else {
    ...
    for (a = 0; a < slCT; a++) {
        newdr[a + 1] = clustemp[a];    //1 - Length field
    }
    ...
    for (a = 0; a < slLT; a++) {
        newdr[a + slCT + 2] = lentemp[a];    //2 - Length field
    }

    for (a = 0; a < slCT; a++) {
        newdr[a + slLT + slCT + 2] = clustemp[a];    //2 - Offset field
    }

    newdr[((slCT) * 2) + slLT + 2] = 0x00 + slCR; //3 - Sparse

    for (a = 0; a < slCR; a++) {
        newdr[((slCT) * 2) + slLT + 3 + a] = clusremaintemp[a];    //3 - Length field
    }
    ...
    for(a = 0; a < newdrlength; a++) {
        filerecord[droffset + a] = newdr[a];
    }

    for(a = 0; a < alignment; a++) {
        filerecord[droffset + newdrlength + a] = 0x00;
    }
    ...
    for(a = 0; a < 4; a++) {
        filerecord[droffset + newdrlength + alignment + a] = 0xFF;
    }
    ...
    for (a = 0; a < 4; a++) {
        filerecord[DATAoff + 4 + a] = temp[a];    //Length of $DATA
    }
    ...
    for (a = 0; a < 4; a++) {
        filerecord[0x18 + a] = temp[a];    //Length of record
    }
...
void altboot (HANDLE hvol, long int bpc, FILE *pfile, long long int MFTstart, long int filesize, int bps, int choice)
{
    ...
    if (datarunLength[0] > filesize) { //Is there enough space?
        ...
        if (sectorskip > (pow(2, 32) - 1)) { //After 2GB mark
            ...
            DATAlo32 = sectorskip;    //Setup lo32 alone
        }
...
DWORD writefileatsetpoint(HANDLE hvol, FILE *pfile, long int filesize, int bps)
{
    ...
    for(int a = 0; a < secfilesize; a++) { //Populate filearray[]
        if (a < filesize) { //Don't write from memory
            ...
            filearray[a] = 0x00;    //Pad array with zeroes
        }
    }
    ...
    } //if(write successful){Inform user;}
...
int DATAattr(unsigned char filerecord[], int ona, long long int datarunLength[], long long int datarunOffset[], long int bpc)
{
    ...
    for (a = 0 ; a < 4; a++)	{
        Hona[a] = filerecord[ona + 4 + a];
    }

    int tona = hextodec(Hona, 4);	//tona = (to) off to next attr

    Hona[0] = filerecord[ona + 0x20];
    Hona[1] = filerecord[ona + 0x21];
    ...
    for (a = 0; a < lodr; a++) {
        datarunarray[a] = filerecord[ona + otdr + a];    //Populate datarunarray[]
    }
    ...
    do {
        ...
        for (b = 0; b < drL; b++)	{
            Hona[b] = datarunarray[b + a + 1];    //Populate Hona[] with length
        }

        datarunLength[c] = (hextodec(Hona, drL) * bpc); //Needs to be bytes per cluster

        for (b = 0; b < drO; b++)	{
            Hona[b] = datarunarray[b + a + drL + 1];    //Populate Hona[] with offset
        }
...
int ATTRfind (unsigned char filerecord[],  int ATTRtype)
{
    ...
    Hona[0] = filerecord[0x14];
    Hona[1] = filerecord[0x15];
